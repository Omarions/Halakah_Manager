...
) ENGINE=InnoDB AUTO_INCREMENT=41 DEFAULT CHARSET=utf8 COMMENT='saves the course details, start time and the subject studied, the teacher and the period of course. The end date is optional because some courses is open.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `course_track` (
  ...
  CONSTRAINT `FK_CT_COURSE` FOREIGN KEY (`course_id`) REFERENCES `course` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE,
...
CREATE TABLE `student_track` (
  ...
  CONSTRAINT `FK_COURSE_STUDENTTRACK` FOREIGN KEY (`course_id`) REFERENCES `course` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE,
